...
bs91_11 <- filter(bs, steuerjahr%in%c(1991,2011)) %.% select(steuerjahr, household, reineinkommen,agegroup,alterskategorie) %.% mutate(steuerjahr=factor(steuerjahr))
...
p50s <- aggregate(log(bs91_11$reineinkommen)~bs91_11$alterskategorie+factor(bs91_11$steuerjahr)+bs91_11$single,FUN=median,na.rm=TRUE)
names(p50s) <- c("alterskategorie","steuerjahr","single","log_income")
p50s <- p50s[order(p50s$alterskategorie),]
theils <- aggregate(bs91_11$reineinkommen~bs91_11$alterskategorie+factor(bs91_11$steuerjahr)+bs91_11$single,FUN=Theil)
names(theils) <- c("alterskategorie","steuerjahr","single","Theil")
theils <- theils[order(theils$alterskategorie),]

ggplot(theils, aes(x=alterskategorie, y=Theil,group=steuerjahr,color=steuerjahr))+geom_line()+
scale_color_manual(values=c("1991"="blue","2011"="red" ))+
facet_wrap(~single)+
theme_bw()
```
...
ggplot(bs_nomiss,aes(x=log(reineinkommen),fill=steuerjahr,weight=w))+
  geom_density(aes(y=..density..),alpha = 0.5,position="identity")+
 #   geom_vline(data=p50s, aes(xintercept=log_income,color=steuerjahr))+
   geom_text(data=theils, position=position_dodge(width=6), aes(x = 1, y = 60000, color=steuerjahr, label = paste("Theil:",round(theils$Theil,3))))+
